...
describe('createLiveBoard', () => {
  ...
  it('holds the previous best driver out of the next rerank after a load rotates', () => {
    const board = createLiveBoard({
      tickMs: 60000,
      tickMinutes: 18,
    })
    ...
    const assignedDriver = snapshot.drivers.find((driver) => driver.status === 'Assigned')
    const decision = buildDeterministicDispatchDecision(snapshot.drivers, snapshot.activeLoad, {
      currentTime: snapshot.clock,
    })

    expect(assignedDriver).toBeDefined()
    expect(decision.recommended.id).not.toBe(assignedDriver.id)
  })
})
