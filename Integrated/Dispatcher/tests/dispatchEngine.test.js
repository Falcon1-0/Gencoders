...
describe('dispatchEngine', () => {
  ...
  it('skips drivers that are already assigned to another load', () => {
    const snapshot = buildDispatchCandidates(
      drivers.map((driver) =>
        driver.id === 'DRV-201'
          ? { ...driver, status: 'Assigned', dispatchLocked: true }
          : driver,
      ),
      defaultLoad,
      {
    ...
    expect(snapshot.candidates.some((candidate) => candidate.id === 'DRV-201')).toBe(false)
  })
})
