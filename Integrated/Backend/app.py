...
# Always load the backend env file relative to this app so ngrok/Twilio config
# works no matter which folder the server is launched from.
...
def describe_dispatch_error(exc: Exception) -> str:
    if isinstance(exc, TwilioRestException):
        if exc.status == 401:
            return "Twilio authentication failed. Check TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN in Backend/.env, make sure they belong to the same Twilio account, then restart the backend."
        if exc.status == 403:
            return "Twilio rejected this request with HTTP 403. Check account permissions, trial restrictions, and whether the destination number is allowed."
        if exc.code:
            return f"Twilio error {exc.code}: {exc.msg}"
    return str(exc)
...
def process_dispatch_job(job_id: str, ranked: list[dict[str, Any]]) -> None:
    ...
    for idx, candidate in enumerate(ranked, start=1):
        ...
        try:
            ...
            import traceback
            user_message = describe_dispatch_error(exc)
            print(f"[ERROR] {driver.name}: {user_message}")
            traceback.print_exc()
            update_candidate(job_id, driver.driver_id, status="failed", transcript=user_message, responded_at=utc_now())
            update_job(job_id, latest_message=f"Call error for {driver.name}: {user_message}")
            continue
