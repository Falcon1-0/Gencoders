...
import { buildDeterministicDispatchDecision } from '../../src/lib/dispatchEngine.js'
...
function syncAssignedDriver(driver, tick, timestamp) {
  if (driver.dispatchLockedUntilTick && tick < driver.dispatchLockedUntilTick) {
    driver.dispatchLocked = true
    driver.status = 'Assigned'
    return null
  }

  if (driver.dispatchLocked) {
    driver.dispatchLocked = false
    delete driver.dispatchLockedUntilTick
    delete driver.assignedLoadId
    ...
    if (previousStatus !== 'Available') {
      return buildEvent(
        `evt-driver-release-${timestamp}-${driver.id}`,
        timestamp,
        `${driver.name} is back on the board`,
        `${driver.name} cleared the prior assignment and is available for the next round of planning.`,
        'green',
      )
    }
  }
...
export function createLiveBoard(options = {}) {
  ...
  function rotateLoad(timestamp) {
    ...
    const assignment = buildDeterministicDispatchDecision(state.drivers, completedLoad, {
      currentTime: state.clock,
    })
    const assignedDriver = state.drivers.find((driver) => driver.id === assignment.recommended?.id)

    if (assignedDriver) {
      assignedDriver.dispatchLocked = true
      assignedDriver.dispatchLockedUntilTick = state.tick + ASSIGNMENT_LOCK_TICKS
      assignedDriver.assignedLoadId = completedLoad.id
      assignedDriver.status = 'Assigned'
      assignedDriver.currentCity = completedLoad.destination
      assignedDriver.hosDriveHours = clamp(
        assignedDriver.hosDriveHours - Math.max(1.5, (assignment.recommended?.loadedDriveHours || 0) * 0.35),
        0.4,
        11,
      )
      ...
      pushEvent(
        buildEvent(
          `evt-assigned-${timestamp}-${assignedDriver.id}`,
          timestamp,
          `${completedLoad.id} assigned to ${assignedDriver.name}`,
          `${assignedDriver.name} was committed to ${completedLoad.origin} to ${completedLoad.destination} and is being held off the next rerank.`,
          'green',
        ),
      )
    }
  ...
  function advanceTick() {
    ...
    state.drivers.forEach((driver) => {
      pushEvent(syncAssignedDriver(driver, state.tick, state.clock))
    })
